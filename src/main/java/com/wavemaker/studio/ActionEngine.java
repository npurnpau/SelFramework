...
import com.wavemaker.support.HtmlReportSupport;
import com.studio.utilities.Reporter;
...
public class ActionEngine extends TestEngine {
	...
	public boolean isElementPresent(By by, String locatorName)
		...
		try {
			...
			if (!flag) {
				...
				SuccessReport("IsElementPresent ",
	...
	public boolean rightclick(By by, String locatorName)
		...
		try {
			...
			if (!flag) {
				failureReport("RightClick ",
						"RightClick action is not perform on " + locatorName);

			} else if (b && flag) {
				SuccessReport("RightClick ",
	...
	public boolean waitForElementPresent(By by, String locator)
		...
		try {
			for (int i = 0; i < 300; i++) {
				if (driver.findElement(by).isDisplayed()) {
					flag = true;
					System.out.println("Wait - Successfully found element "+ locator);
					return true;
	...
	public boolean selectByVisibleText(By locator, String visibletext,
		...
		try {
			...
			if (!flag) {
				failureReport("Select ", visibletext
						+ " is Not Select from the DropDown " + locatorName);

			} else if (b && flag) {
				SuccessReport("Select ", visibletext
						+ "  is Selected from the DropDown " + locatorName);
			}
		}
	}
...
//			}
	...
	public boolean launchUrl(String url) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("Launching URL ", "Failed to launch "
						+ url);
			} else if (b && flag) {
				SuccessReport("Launching URL ",
	...
	public boolean isChecked(By locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				SuccessReport("IsChecked ", locatorName
						+ " is Selected ");
				// throw new ElementNotFoundException("", "", "");

			} else if (b && flag) {
				failureReport("IsChecked ", locatorName
						+ " is not Select ");
			}
		}
		return bvalue;
	...
	public boolean isEnabled(By locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				failureReport("IsEnabled ", locatorName
						+ " is not Enabled");
	...
	public boolean asserTitle(String title) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("AsserTitle ",
						"Page title is not matched with " + title);
				return false;
			} else if (b && flag) {
				SuccessReport("AsserTitle ",
	...
	public boolean verifyTitle(String title) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("VerifyTitle ",
						"Page title is not matched with " + title);

			} else if (b && flag) {
				SuccessReport("VerifyTitle ",
						" Page title is verified with " + title);
	...
	public String getText(By locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				warningReport("GetText ", " Unable to get Text from "
						+ locatorName);
			} else if (b && flag) {
				SuccessReport("GetText ", " Able to get Text from "
						+ locatorName);
			}
		}
		return text;
	...
	public String getValue(String locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				failureReport("GetValue ", " Unable to get Text from "
						+ locatorName);
			} else if (b && flag) {
				SuccessReport("GetValue ", " Able to get Text from "
						+ locatorName);
			}
		}
		return text;
	...
	public boolean switchToFrameByIndex(int index) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ", " Frame with index "
						+ index + " is not selected");
			} else if (b && flag) {
				SuccessReport("SelectFrame ", " Frame with index "
						+ index + " is selected");
			}
		}
	}
	...
	public boolean switchToFrameById(String idValue) throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ", " Frame with Id "
						+ idValue + " is not selected");
			} else if (b && flag) {
				SuccessReport("SelectFrame ", " Frame with Id "
						+ idValue + " is selected");
			}
		}
	}
	...
	public boolean switchToFrameByName(String nameValue)
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ", " Frame with Name "
						+ nameValue + " is not selected");
			} else if (b && flag) {
				SuccessReport("SelectFrame ", " Frame with Name "
	...
	public boolean switchToDefaultFrame() throws Throwable {
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ",
						" The Frame is not selected");
			} else if (b && flag) {
				SuccessReport("SelectFrame ",
	...
	public boolean switchToFrameByLocator(By lacator, String locatorName)
		...
		try {
			...
			if (!flag) {
				failureReport("SelectFrame ", " The Frame "
	...
	public boolean click1(WebElement locator, String locatorName)
		...
		try {
			...
			if (!flag) {
				failureReport("Click ", " Unable to click on "
						+ locatorName);
				return false;
			} else if (b && flag) {
				SuccessReport("Click ", " able to click on "
						+ locatorName);
				return true;
			}
		}
	...
	public boolean waitForVisibilityOfElement(By by, String locator)
		...
		try {
			...
			if (!flag) {
				failureReport("WaitForVisibilityOfElement ", " Element "
						+ locator + " is not visible");
			} else if (b && flag) {
				SuccessReport("WaitForVisibilityOfElement ", " Element "
						+ locator + "  is visible");
			}
		}
	}
	...
	public boolean waitForInVisibilityOfElement(By by, String locator)
		...
		try {
			...
			if (!flag) {
				failureReport("WaitForInVisibilityOfElement ",
						" Element  " + locator + " is  visible");
			} else if (b && flag) {
				SuccessReport("WaitForInVisibilityOfElement ",
